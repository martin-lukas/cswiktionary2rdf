...
public class PrepositionSectionExtractor implements SectionExtractor {
    
    @Override
    public void extract(Model model, Resource pageResource, Page page) {
        String suffix = TextUtils.connectString(PartOfSpeech.PREPOSITION.getLocalMapping().toLowerCase());
        ...
        new PartOfSpeechModeller().model(model, resource, PartOfSpeech.PREPOSITION);
    }
}
