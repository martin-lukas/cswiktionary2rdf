...
public class ConjunctionSectionExtractor implements SectionExtractor {
    @Override
    public void extract(Model model, Resource pageResource, Page page) {
        String suffix = TextUtils.connectString(PartOfSpeech.CONJUNCTION.getLocalMapping().toLowerCase());
        ...
        new PartOfSpeechModeller().model(model, resource, PartOfSpeech.CONJUNCTION);
    }
}
